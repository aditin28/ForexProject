...
public class UsersController {
	...
	@Autowired
	 ...
	 private UsersAuthenticationService usersAuthenticationService;
	  ...
	  @PostMapping("/login") public ResponseEntity<CommonDTO<Users>>
	 doLogin(@RequestBody LoginDto loginReq) { Users users =
	  usersAuthenticationService.login(loginReq.getEmail(),
	  loginReq.getPassword()); return new ResponseEntity<>(new
	  CommonDTO<>("Auth successfully", users), HttpStatus.OK); }
	  ...
	  @PutMapping("/forgot-password")
		public ResponseEntity<CommonDTO<String>> forgotPassword(@RequestBody ForgotPassDTO r) throws InvalidInputException {
			...
			if (Boolean.FALSE.equals(isOtpVerify))
	  ...
	  @PutMapping("/reset-password") public ResponseEntity<CommonDTO<String>>
	  resetPassword(@RequestBody ResetPasswordDTO r) throws InvalidInputException {
	  
	  return ResponseEntity.ok(new
	  CommonDTO<>(usersService.resetPassword(r.getEmail(),r.getOldPassword(),r.
	 getNewPassword()),usersService.resetPassword(r.getEmail(),r.getOldPassword(),
	 r.getNewPassword()))); }
	 
	  
	  @PostMapping("/sendEmail") public ResponseEntity<CommonDTO<String>>
	  sendEmail(@RequestBody SendOtpDTO r) { var res =
	  usersAuthenticationService.sendEmail(r.getEmail()); return
	  ResponseEntity.ok(new CommonDTO<>(res,res)); }
